...
import { PlusIcon, EditIcon, TrashIcon, UploadIcon, RupeeIcon, EyeIcon, CoinsIcon } from './common/Icons.js';
import { calculateXIRR } from '../utils/finance.js';
import { parseCamsStatement } from '../utils/casParser.js';

// --- Helper Modals ---

const AddFundModal = ({ onSave, onClose }) => {
                    ...
                    <div>
                     ...
                     <div>
                        <label className="text-sm">Owner</label>
                         <select value={owner} onChange={e => setOwner(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-700 p-2 rounded-md mt-1">
...
const AddLumpsumModal = ({ fund, onSave, onClose }) => {
...
const UpdateValueModal = ({ fund, onSave, onClose }) => {
...
const TransactionHistoryModal = ({ fund, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
...
const ImportCasModal = ({ onImport, onClose }) => {
...
const Portfolio = ({ funds, setFunds }) => {
    ...
    const handleUpdateValue = (fundId, newValue) => {
        setFunds(prev => prev.map(f => f.id === fundId ? { ...f, currentValue: newValue } : f));
        setModal('none');
        setSelectedFund(null);
    };

    const handleAddLumpsum = (fundId, transaction) => {
    ...
    const handleImportFunds = (parsedFunds) => {
        setFunds(prevFunds => {
    ...
    const getFundXIRR = (fund) => {
        const cashflows = fund.transactions.map(tx => ({
            amount: -tx.amount,
